...
function Card(props) {
  ...
  const cardDeleteButtonClassName = `button ${
    isOwn ? "elements__delete-button" : "elements__delete-button_none"
  }`;
  // Определяем, есть ли у карточки лайк, поставленный текущим пользователем
  const isLiked = props.card.likes.some((i) => i === currentUser._id);

  // Создаём переменную, которую после зададим в `className` для кнопки лайка
  const cardLikeButtonClassName = `button elements__like-button ${
    isLiked ? "elements__like-button_active" : "button elements__like-button"
  }`;
      ...
      <div className="elements__container">
        ...
        <div className="elements__like">
          <button
            ...
            aria-label="Like"
          ></button>
          <span className="elements__like-amount">
